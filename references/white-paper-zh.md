...
| 生成速度（DS-V2-Lite） | 65 t/s | **145.63 t/s** | 224%（反超全 GPU） |
| 生成速度（Qwen3.6-A3B） | — | **74.99 t/s** | 超 pre-lazy 46.5 |
| V4-Flash（157B/85GB，4090） | OOM | **35.96 t/s** | RSS 17.5-26.8GB（on-demand/selective pin） |
| 模型/显存比 | 0.26× | **3.9×** | +15 倍 |

> **长上下文补充实测（2026-08-09，v5 默认 whole-pin，RTX 4090，-c 8192，3200 tokens 长生成）**：Qwen3.6-A3B **45.1 t/s @ 2.4 GB（2477 MiB）**、DS-V2-Lite **34.1 t/s @ 3.4 GB（3441 MiB）**——8K 上下文 + 长文生成更贴近真实使用；短上下文短生成可达上表 74.99 / 145.63 t/s（2026-08-10 bins-v0.4.0 实测）。原始采样数据与生成全文见演示素材包（rec_data.csv / rec_full.txt）。
...
2. **速度可用**：75-146 t/s 的生成速度（4090，2026-08-10 bins-v0.4.0 实测；2080 Ti 上 47-87 t/s），远超阅读速度，对话体验流畅
...
**为什么快**：专家权重驻留 CPU 内存（mmap 惰性 + on-demand pin 注册），每步只拷激活的 top-k 专家（如 DS 每层 6 个 × 1.55 MB ≈ 9.3 MB），而非全部 64 个专家——拷贝量与激活数量成正比，而非模型大小。实测提升（2026-08-10，bins-v0.4.0，4090）：DS 37.9 → 145.63 t/s（+284%），Qwen 50.2 → 74.99 t/s（+49%）。
...
| 4 | **selective pin（08-10 当前主线）** | 1.6-2.9 GB（V4 16.5-16.7GB VRAM） | **Qwen 74.99 / DS 145.63 / V4 34.67-35.96 t/s** | ✅ 已验证（当前主线） |
...
- DS-V2-Lite：速度 **+284%**（37.9 → 145.63 t/s，bins-v0.4.0 08-10）
- Qwen3.6-A3B：速度 **+49%**（50.2 → 74.99 t/s，超 pre-lazy 46.5）
...
|------|-----------|----------------------|------|
| 显存占用 | 23.3 GB | **1.6-4.9 GB** | **-79%** |
| Gen 速度 | 65 t/s | **145.63 t/s** | 224%（反超） |
| 模型/显存比 | 0.26× | **3.9×** | +15 倍 |
...
|------|--------|--------|---------|
| 显存 | 23.3 GB | 1.6-4.9 GB | 8 GB 卡可跑，显存是硬约束 |
| 速度 | 65 t/s | 145.63 t/s（DS，08-10） | 反超全 GPU，远超流畅门槛 |
...
| 速度（122B 级） | 16.53 t/s @ 20.6 GiB | Qwen3.6-A3B 74.99 t/s @ 3.1 GiB（08-10） |
| 安装 | 源码编译（CMake + C++） | **pip install** |
| 模型支持 | Qwen 系列 | **任意 llama.cpp MoE**（DeepSeek/Qwen/Mixtral） |
| 目标用户 | 移动端 / 边缘 | **桌面 / 家庭服务器** |

**启示**：两家独立验证了"专家 offload + LRU 缓存 + 预取"路线的有效性。mollm 用 1 GB 缓存 + 5.9 GiB 总内存跑 122B 模型（12.38 t/s），moe-l2 用 1.6-4.9 GB 显存跑 16B MoE（145.63 t/s，08-10）——在各自平台上都证明**消费级硬件可以跑大 MoE**。
