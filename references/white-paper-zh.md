...
| V4-Flash（157B/85GB，4090） | OOM | **30.9 t/s** | RSS 84.4→10.4GB（↓88%） |
...
| 4 | **selective pin（08-10 当前主线）** | 1.6-2.9 GB（V4 10.4GB RSS） | **Qwen 50.2 / DS 37.9 / V4 30.9 t/s** | ✅ 已验证（当前主线） |
...
| **V4-Flash** | **selective pin（08-10）** | — | **30.9** | **10.4 GB RSS** |

- DS-V2-Lite：速度 **+200%**（12.5 → 37.9 t/s），显存几乎不变
- Qwen3.6-A3B：速度 **+400%**（10 → 50.2 t/s，超 pre-lazy 46.5）
- V4-Flash：**selective pin 后 RSS 84.4 → 10.4GB（↓88%）**，速度 30.9 t/s（此前 10.1 t/s 为官方原版二进制，moe-l2 优化版本来就是 ~30 t/s），4090 上近计算 bound
