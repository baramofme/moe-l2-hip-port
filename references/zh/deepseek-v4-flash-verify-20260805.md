# DeepSeek V4 Flash 验证报告（更新至 2026-08-14；⚠️ 速度数据 P0 假速度 + 量化乱码，作废待重测）

> ⚠️ **2026-08-10 4090 复测（bins-v0.4.0）速度数据作废**：V4 速度 35.96 / 34.67 t/s 为 **UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版**（V4 乱码是模型/量化问题，非 moe-l2 问题，原版也乱码；且 2026-08-13 定性 P0 竞态假速度）。RSS 17.5/26.8GB、VRAM 16.5-16.7GB 为内存实测仍可参考。

> ⚠️ **2026-08-10 基准更正（再更正）**：本报告 10.1 t/s 为**官方原版 llama.cpp 二进制**实测；moe-l2 优化版二进制在 RTX 4090 上曾报 **35.96 t/s**、RSS **17.5-26.8GB**——⚠️ 该速度现定性为 **UD-IQ2_M 2bit 量化退化乱码 + P0 假速度，无有效速度数据，等 Q4 量化版**。本报告下方 08-05/08-07 数据保留作历史记录。
...
## 最新成果（2026-08-10）：bins-v0.4.0 全链路（⚠️ 速度数据作废，等 Q4 量化版）

> **bins-v0.4.0（selective pin + on-demand 兜底）曾报 35.96 t/s（vs 原版二进制 10.1 t/s）——⚠️ 该速度为 P0 假速度 + UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版**；RSS 从 whole-pin 84GB 降到 17.5-26.8GB 仍有效。90GB 模型在 24GB 卡 + 1TB 内存机器上可运行。

### 一句话结论

**moe-l2 在 RTX 4090 上用 bins-v0.4.0 跑 DeepSeek V4 Flash（UD-IQ2_M，85GB 三片）：RSS 17.5-26.8GB、VRAM 16.5-16.7GB；⚠️ 速度 35.96 / 34.67 t/s 为 UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版**（2026-08-10 实测记录，完整 4 轮数据见文末"08-10 复测"节）。

### 实测（RTX 4090 24GB，2026-08-10，全链路 `moe-l2 start --gpu`，round3 稳定轮）

| 形态 | Gen t/s | VRAM | RSS |
|------|---------|------|-----|
| on-demand 兜底（auto 路由表） | ⚠️ 35.96（UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版） | 16.5 GB | 17.5 GB |
| selective pin（v4_top100.map） | ⚠️ 34.67（同上，无有效速度数据） | 16.7 GB | 26.8 GB |
| 原版 llama.cpp 二进制（对照） | 10.1（同样乱码，仅作对照） | 17.4 GB | 82 GB |

### 关键结论（2026-08-10）

1. ⚠️ **V4 速度数据全部作废**：34-36 t/s（及 30.9 等）为 UD-IQ2_M 2bit 量化退化乱码 + P0 假速度，无有效速度数据，等 Q4 量化版重测
2. **RSS 大幅缩减**：whole-pin 84GB → on-demand 17.5GB（↓79%）/ selective pin 26.8GB（↓68%）（内存实测仍有效）
3. **10.1 t/s 是原版二进制的数据**：原版同样乱码，仅作对照
...
**实测（短上下文，c=512）**：RTX 4090 + moe-l2（on-demand pin + A3 cache 2048）下 **10.1 t/s**（2026-08-07，原版二进制口径；08-10 曾报 35.96 t/s——⚠️ P0 假速度 + 量化乱码，无有效速度数据，等 Q4 量化版）。
...
- 85GB 三片 GGUF 在 2080 Ti（11GB）上跑通：显存 8.4GB / 11GB、RSS 靠专家页淘汰 v3.1 封顶（11-12GB，对比无淘汰 29GB）——**验证"10-11GB 卡可跑 85GB 模型"可行性**（08-10 曾报 4090 上 35.96 t/s，⚠️ 假速度/量化乱码作废，等 Q4 量化版）
...
### 实测（RTX 4090，2026-08-10，-c 8192，⚠️ P0 假速度，2026-08-13 作废）
...
1. ⚠️ **V4 速度数据作废**：34-36 t/s（及 30.9 t/s）为 P0 假速度 + UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版
2. **RSS 大幅缩减**：whole-pin 84GB → on-demand 17.5GB（↓79%）/ selective pin 26.8GB（↓68%）
3. 阶段1 的 10.4GB RSS 是纯 selective pin 无 prefill 配置；本次含 GPU cache 预填充所以 RSS 更高（⚠️ "速度更快"基于 P0 假速度，作废）
