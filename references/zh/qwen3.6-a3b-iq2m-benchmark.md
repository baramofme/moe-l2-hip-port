# Qwen3.6-35B-A3B-UD-IQ2_M benchmark 报告（更新至 2026-08-14，P0 假速度数据已标注作废）

## 最新成果（2026-08-14）：修复版全链路复测（bins-v0.4.1）

> ⚠️ 2026-08-10 bins-v0.4.0 三卡数据为 P0 竞态 bug 假速度（2026-08-13 定性作废）；2026-08-14 bins-v0.4.1 修复版全链路重测：4090 Qwen **44-48 t/s**（本次复测 36-46 吻合）、2080 Ti Qwen **30.87 t/s（有锁版）**（5090 待重测），均远超官方原版 llama.cpp 二进制（9.71-11.15 t/s 级别）。

### 实测（2026-08-10，全链路 selective pin，round3 稳定轮）

| GPU | 短对话 | 追问1 | 追问2 | VRAM | RSS |
|-----|--------|-------|-------|------|-----|
| RTX 2080 Ti | 待重测 | **30.87**（修复版，有锁版） | 待重测 | 待补测 | 待补测 |
| RTX 4090 | 36-46（本次复测） | **44-48**（修复版） | 待重测 | ~9.3 GB | ~11.4 GB |
| RTX 5090 | — | ⚠️ 76.41（P0 假速度，待重测） | — | ~1.3-2.5 GB | — |
...
| 轮次 | 短对话 | 追问1 | 追问2 |
|------|--------|-------|-------|
| Round 1（冷启动） | 36.85 | 47.01 | 51.52 |
| Round 2 | 57.78 | 71.93 | 62.49 |
| Round 3（稳定） | **65.08** | **74.99** | **63.71** |
...
| 轮次 | 短对话 | 追问1 | 追问2 |
|------|--------|-------|-------|
| Round 1（冷启动） | 14.59 | 58.66 | 55.93 |
| Round 2 | 61.44 | 78.40 | 68.85 |
| Round 3（稳定） | **66.44** | **76.41** | **69.86** |
| Round 4 | 66.78 | 76.45 | 68.19 |

- 口径：`python3 speed_test.py 11435`（64 tok/请求，proxy 全链路含路由表 + selective pin）
- 稳定值（⚠️ P0 假速度，2026-08-13 作废）：2080 Ti ~41-47、4090 ~64-75、5090 ~66-76 t/s；修复版：2080 Ti 30.87（有锁版）、4090 44-48（5090 待重测）

### 关键结论（2026-08-10）

1. **2080 Ti 上 Qwen 全链路 30.87 t/s**（2026-08-14 修复版，有锁版；旧 47.24 为 P0 假速度作废；vs 原版 llama.cpp 11.15 t/s ≈ +177%）
2. **5090 上 Qwen ⚠️ P0 假速度（76.41 t/s 作废），待重测**——2026-08-13 定性
3. **4090 上 Qwen 全链路 44-48 t/s**（2026-08-14 bins-v0.4.1 修复版，round3 追问1；本次复测 36-46 吻合；旧 74.99/63.71 为 P0 假速度作废；与 pre-lazy 46.5 相当；VRAM ~9.3GB、RSS ~11.4GB）
4. **selective pin 零拖累**：路由表 top-100 pin 后 2080 Ti 速度与 whole-pin 同量级（修复版 30.87 t/s）
5. **冷启动明显**：round1 26.75 → round3 41.66（首次加载路由表 + 专家 pin + GPU cache 预热；P0 假速度期间数据，趋势仍成立）
