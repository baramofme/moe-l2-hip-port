...
moe-l2 是一个面向消费级 NVIDIA 显卡的 MoE（混合专家）模型推理加速方案，核心能力是**把原本需要 16-24 GB 显存的模型压缩到约 10 GB 运行**（4090 修复版实测：DS ~10 GB、Qwen ~9.3 GB；8K 长上下文占用待重测），速度达到全 GPU 形态的约 2 倍。

核心数字（RTX 4090 实测，2026-08-14，bins-v0.4.1 修复版全链路）：

| 指标 | 全 GPU 形态 | moe-l2 | 变化 |
|------|------------|--------|------|
| 显存占用 | 23.3 GB | **~10 GB（DS）/ ~9.3 GB（Qwen）** | **-57%** |
| 生成速度（DS-V2-Lite） | 65 t/s | **133.2 t/s** | 205%（反超全 GPU） |
| 生成速度（Qwen3.6-A3B） | — | **44-48 t/s** | ≈ pre-lazy 46.5 |
| V4-Flash（157B/85GB，4090） | OOM | ⚠️ 35.96 t/s（UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版） | RSS 17.5-26.8GB（on-demand/selective pin） |
| 模型/显存比 | 0.26× | **3.9×** | +15 倍 |

> **长上下文（c=8192）实测——2026-08-14 修复版（bins-v0.4.1，RTX 4090）**：Qwen3.6-A3B **39.6-42.7 t/s @ ~8.9 GB**、DS-V2-Lite **119.0-123.6 t/s @ ~10.1 GB**（8K 长文生成 + 追问；旧 08-09 v5 的 45.1/34.1 t/s 为 P0 假速度作废）。8K 上下文 + 长生成更接近真实使用；短上下文短生成见上表 133.2 / 44-48 t/s（2026-08-14 bins-v0.4.1 修复版实测）。原始采样数据与生成全文见演示素材包（rec_data.csv / rec_full.txt）。

> **多架构全兼容（2026-08-14，bins-v0.4.1 修复版）**：一个二进制支持所有 NVIDIA 消费卡（GTX 1080 sm_61 → RTX 50 系 sm_120a，CUDA 12.8 编译）。2080 Ti 全链路实测（修复版 + selective pin）：DS-V2-Lite **85.25 t/s**、Qwen3.6-A3B **30.87 t/s（有锁版）**（2026-08-14；此前 6.89/11.15 为官方原版二进制数据，87.25/47.24 为 P0 假速度作废）。完整报告见 [multi-arch-three-gpu-benchmark.md](multi-arch-three-gpu-benchmark.md)。v0.4.1 修复 P0 竞态 bug。
...
1. **显存降 57%**：23.3 GB → ~10 GB（4090 修复版实测，DS）；Qwen ~9.3 GB——8 GB 卡跑 32B MoE 需谨慎评估（长上下文占用待重测）
2. **速度可用**：44-133 t/s 的生成速度（4090，2026-08-14 bins-v0.4.1 修复版实测；2080 Ti 上 31-85 t/s），远超阅读速度，对话体验流畅
...
**为什么快**：专家权重驻留 CPU 内存（mmap 惰性 + on-demand pin 注册），每步只拷激活的 top-k 专家（如 DS 每层 6 个 × 1.55 MB ≈ 9.3 MB），而非全部 64 个专家——拷贝量与激活数量成正比，而非模型大小。实测提升（2026-08-10 bins-v0.4.0 为 P0 假速度作废；2026-08-14 bins-v0.4.1 修复版，4090）：DS 37.9 → 133.2 t/s（+251%）；Qwen 44-48 t/s（旧 74.99 为 P0 假速度作废，修复后与 pre-lazy 46.5 相当）。
...
| 4 | **selective pin（08-14 修复版当前主线）** | ~9.3-10 GB（4090 修复版实测；V4 16.5-16.7GB VRAM） | **Qwen 44-48 / DS 133.2 t/s**（V4 ⚠️ 量化乱码无有效速度，等 Q4 量化版） | ✅ 已验证（当前主线） |
...
### 4.1 核心结果：host buffer → on-demand pin → selective pin 演进
...
| **DS-V2-Lite** | **selective pin（08-14 修复版）** | — | **133.2**（旧 145.63/127.95 为 P0 假速度作废） | **~10 GB / ~6.7 GB RSS** |
| Qwen3.6-A3B（32B MoE，256 专家） | CPU buffer（旧） | 10.0 | 10.0 | 2141 MiB |
| **Qwen3.6-A3B** | **host buffer** | **75.8** | **46.8** | **2147 MiB** |
| **Qwen3.6-A3B** | **selective pin（08-14 修复版）** | — | **44-48**（旧 74.99/63.71 为 P0 假速度作废） | **~9.3 GB / ~11.4 GB RSS** |
| V4-Flash（157B/85GB） | lazy 无 pin（08-05） | — | 1.7-2.0 | 9.1 GB |
| **V4-Flash** | **selective pin / on-demand（08-10）** | — | ⚠️ 34.67 / 35.96（UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版） | **26.8 / 17.5 GB RSS** |

- DS-V2-Lite：速度 **+251%**（37.9 → 133.2 t/s，bins-v0.4.1 08-14 修复版；旧 145.63 为 P0 假速度作废）
- Qwen3.6-A3B：**44-48 t/s**（旧 74.99 为 P0 假速度作废，修复后与 pre-lazy 46.5 相当）
- V4-Flash：**RSS 84.4 → 17.5-26.8GB（↓68~79%）**；⚠️ 速度 34.67-35.96 t/s 为 UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版（此前 10.1 t/s 为官方原版二进制）
...
### 4.3 全量 vs moe-l2 对比（DS-V2-Lite Q2_K，selective pin 主路径）

| 指标 | 标准全 GPU | moe-l2（selective pin，08-10） | 变化 |
|------|-----------|----------------------|------|
| 显存占用 | 23.3 GB | **~10 GB** | **-57%** |
| Gen 速度 | 65 t/s | **133.2 t/s** | 205%（反超） |
| 模型/显存比 | 0.26× | **3.9×** | +15 倍 |
...
> ⚠️ 适配矩阵基于 P0 假数据时期的显存占用（1.6-4.9GB）；修复版 4090 实测 ~9.3-10GB，4GB/6-8GB 卡适配需以 bins-v0.4.1 重测。
...
|------|--------|--------|---------|
| 显存 | 23.3 GB | ~9.3-10 GB（08-14 修复版实测） | 显存是硬约束；8 GB 卡需谨慎评估 |
| 速度 | 65 t/s | 133.2 t/s（DS，08-14 修复版） | 反超全 GPU，远超流畅门槛 |
| 硬件门槛 | 24 GB 卡（8000+ 元） | 10 GB 级显存需求（08-14 修复版实测） | 成本大降 |
...
| 速度（122B 级） | 16.53 t/s @ 20.6 GiB | Qwen3.6-A3B 44-48 t/s @ ~9.3 GiB（08-14 修复版） |
| 安装 | 源码编译（CMake + C++） | **pip install** |
| 模型支持 | Qwen 系列 | **任意 llama.cpp MoE**（DeepSeek/Qwen/Mixtral） |
| 目标用户 | 移动端 / 边缘 | **桌面 / 家庭服务器** |

**启示**：两家独立验证了"专家 offload + LRU 缓存 + 预取"路线的有效性。mollm 用 1 GB 缓存 + 5.9 GiB 总内存跑 122B 模型（12.38 t/s），moe-l2 用 ~10 GB 显存跑 16B MoE（133.2 t/s，08-14 修复版）——在各自平台上都证明**消费级硬件可以跑大 MoE**。
