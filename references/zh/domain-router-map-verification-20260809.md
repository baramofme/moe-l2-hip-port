# 领域路由表方案验证报告（Qwen + V4 双模型实测；2026-08-14 更新：P0 假速度数据已标注作废）
...
|---|---|---|---|---|---|
| A：选择性 pin（top-100） | **10.4 GB** | 10.71 | 30.14 | 30.88 | ⚠️ 30.9 t/s（P0 假速度 + UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版） |
| B：whole-pin（对照） | 84.4 GB | 24.45 | 31.23 | 30.21 | ⚠️ 30.2 t/s（同上，无有效速度数据） |

### 关键结论
1. **RSS 84.4 → 10.4GB（↓ 88%）**（内存实测仍有效）；⚠️ 速度 30.9 vs 30.2 t/s 为 P0 假速度 + UD-IQ2_M 2bit 量化退化乱码，无有效速度数据，等 Q4 量化版——「内存降低」成立，「速度不掉」待重测
2. **启动更快**：选择性 pin 10s 就绪 vs whole-pin 40s（不 fault 全量页）
3. **⚠️ 速度 30 t/s 为 P0 假速度 + 量化乱码（2026-08-13 定性作废）**——A/B 两组 30 t/s 均不可信；选择性 pin 的贡献 = 内存 88% 缩减（仍有效），速度影响待修复版重测
...
## 阶段 2 实验：GPU cache 预填充（2026-08-10 下午，4090 云机全链路，⚠️ 本节速度为 P0 假速度，2026-08-13 作废，相对趋势仅供参考）
...
### 结论
1. ⚠️ **预填充相对趋势参考**：冷启动 round1 +78%（10.7→19.1，P0 假速度口径），稳态 ~30 t/s 不变（同样作废，等 Q4 量化版重测）
2. **用户判定（2026-08-10 晚）：重要，不是"价值有限"**——首请求即满速 = 打开应用/演示时第一句话就快，直接提升体验感。对 AI PC 厂商 PoC 演示、短 prompt 单请求、频繁重启场景尤其关键（⚠️ 该判定基于 P0 假速度，待修复版重测后复核）
...
> ⚠️ 下表 2026-08-10 数据为 P0 假速度（2026-08-13 作废）；修复版稳定值 **30.87 t/s**（有锁版）。
...
> 📌 **2026-08-14 修复版复测**：Qwen **30.87 t/s**、DS **85.25 t/s**（见 qwen3.6-a3b-iq2m-benchmark.md / deepseek-v2-lite-q2k-benchmark.md 08-14 章节）。⚠️ 上表与 08-10 晚复测（47.24/87.25）均为 P0 假速度，2026-08-13 作废；README/models-benchmark 采用修复版值。

### DS-V2-Lite（Q2_K，codegen 128 tokens）

> ⚠️ 下表 2026-08-10 数据为 P0 假速度（2026-08-13 作废）；修复版稳定值 **85.25 t/s**。
...
- Qwen：42.97 / 58.85 / **62.40 t/s**（⚠️ P0 假速度，作废）
- DS V2：60.09 / 101.01 / **104.85 t/s**（⚠️ P0 假速度，作废）
- 全链路 vs 裸 server 差异 ~15%（proxy 转发 + 域预测开销，正常；修复版待重测）

### 结论

1. **moe-l2 优化版多架构二进制在 2080Ti 上：Qwen ~2.8x、DS ~12x 提升**（修复版 30.87/85.25 vs 原版 11.15/6.89；旧 3.5x/14x 基于 P0 假速度作废）
...
4. **发布基准更新（修复版）**：2080Ti 上 Qwen 30.87 t/s（有锁版）、DS 85.25 t/s；4090 上 V4 ⚠️ 量化乱码无有效速度（等 Q4 量化版）
