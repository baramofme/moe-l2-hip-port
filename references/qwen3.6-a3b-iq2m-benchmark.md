...
原始模式（不传 `--cpu-moe`，**默认 mmap**）：A3 调度开启 + **专家计算在 GPU cuBLAS** 上完成。专家权重通过 mmap 留在 CPU RAM，GPU 仅放非专家层（~2.1 GB），非「全量加载」。
A3 模式（传 `--cpu-moe`）：A3 调度开启 + **专家计算在 CPU** 上完成。

注意：GGUF 文件本身已内置 A3 架构（35B 总参数，3.6B 活跃），两种模式都走 A3 路由调度，区别只在专家计算位置。

> **注意**：原始的 mmap 默认模式≠`--no-mmap` 全量加载。`--no-mmap` 会把全部模型权重塞进 GPU VRAM，Qwen3.6-A3B IQ2_M 约 **~8 GB**。A3 对比的目标是那种全量加载场景，而不是 mmap 默认的 2.1 GB。
...
3. **VRAM 对比**：原始 (mmap 默认) 反而比 A3 低（2.1 GB vs 3.3 GB）。因为 mmap 默认下专家权重在 CPU RAM（仅非专家层 ~2.1 GB 上 GPU），而 `--cpu-moe` 需要额外分配 CPU↔GPU 传输缓冲区和 LRU 缓存管理结构，反而多占 ~1.2 GB。
