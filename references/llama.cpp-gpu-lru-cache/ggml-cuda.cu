...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // triggering the H2D swap pipeline (A3) even when VRAM is sufficient
    {
        ...
        if (force_cpu_experts && !experts_on_host) {
            ...
            if (it == d2h_cache.end()) {
                ...
                char * host_buf;
                CUDA_CHECK(cudaMallocHost(&host_buf, total));
                CUDA_CHECK(cudaMemcpy(host_buf, src0->data, total, cudaMemcpyDeviceToHost));
    ...
    // LRU expert cache (Phase 3): lazy init from GGML_CUDA_EXPERT_CACHE env var

    ggml_cuda_expert_cache_maybe_init(ne02);
    if (experts_on_host) {
        // Build list of used expert indices
        ...
        // Non-swap path: expert weights already on GPU
        for (int64_t i02 = 0; i02 < ne02; ++i02) {
            if (tokens_per_expert[i02] == 0) {
                continue;
            }
