...
// [moe-l2] ff-dyn router-table swap API, exported from ggml-backend.cpp
extern "C" void ggml_backend_router_clear();
extern "C" void ggml_backend_router_set_layer(int layer, const int32_t * experts, int n);
#include <signal.h>
...
int llama_server(common_params & params, int argc, char ** argv) {
    ...
    // [moe-l2] per-domain router table switch (CUDA parity): swap which
    // experts participate in the VRAM expert cache. Body: {domain, experts:{layer:[eids]}}
    ctx_http.post("/moe-set-domain", ex_wrapper([](const server_http_req & req) -> server_http_res_ptr {
        ...
        try {
            ...
            res->status = 400;
            res->data   = "{\"error\":\"invalid JSON\"}";
            return res;
        }
        if (!body.contains("domain") || !body.contains("experts") || !body["experts"].is_object()) {
            res->status = 400;
            res->data   = "{\"error\":\"missing domain or experts\"}";
            return res;
        }
        const std::string domain = body["domain"].get<std::string>();
        ggml_backend_router_clear();
        int total_experts = 0;
        for (auto it = body["experts"].begin(); it != body["experts"].end(); ++it) {
            const int layer = std::atoi(it.key().c_str());
            if (layer < 0 || !it.value().is_array()) continue;
            std::vector<int32_t> ids;
            for (const auto & e : it.value()) {
                if (e.is_number_integer()) {
                    ids.push_back((int32_t) e.get<int>());
                }
            }
            ggml_backend_router_set_layer(layer, ids.data(), (int) ids.size());
            total_experts += (int) ids.size();
        }
        SRV_INF("moe-set-domain: domain=%s experts=%d\n", domain.c_str(), total_experts);
        res->status = 200;
        res->data   = json{{"ok", true}, {"domain", domain}, {"experts", total_experts}}.dump();
        return res;
    }));
