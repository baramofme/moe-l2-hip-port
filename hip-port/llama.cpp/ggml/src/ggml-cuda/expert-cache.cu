...
const void * ggml_cuda_expert_cache_get(const void * cpu_src) {
    static int g_cnt = 0; static int g_hit = 0; static int g_miss = 0;
    g_cnt++;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (slot.valid.load(std::memory_order_acquire) && slot.cpu_src == cpu_src) {
            ...
            if (slot.valid.load(std::memory_order_relaxed) && slot.cpu_src == cpu_src) {
                ...
                g_hit++;
                if ((g_cnt % 200) == 0) {
                    fprintf(stderr, "[HIP-CACHE-GET] cnt=%d hit=%d miss=%d hitrate=%.1f%%\n",
                        g_cnt, g_hit, g_miss, 100.0*g_hit/(g_hit+g_miss));
                }
                return p;
            }
            slot_unlock(slot.lock);
